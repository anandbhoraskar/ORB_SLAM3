...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, const string &_nameSeq):
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr), mbPrunePoints(false)
{
...
bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings)
{
    ...
    if(mSensor==System::STEREO || mSensor==System::RGBD || mSensor==System::IMU_STEREO)
    {
        ...
        if(!node.empty()  && node.isReal())
        {
            ...
            cout << endl << "mbf:" << mbf << "fx:" << fx << ", Depth Threshold (Close/Far Points): " << mThDepth << endl;
        }
        else
        {
...
void Tracking::SearchLocalPoints()
{
    ...
    for(vector<MapPoint*>::iterator vit=mvpLocalMapPoints.begin(), vend=mvpLocalMapPoints.end(); vit!=vend; vit++)
    {
        ...
        if(mCurrentFrame.isInFrustum(pMP,0.5, mImDepth))
        {
            pMP->IncreaseVisible();
            nToMatch++;
        }
