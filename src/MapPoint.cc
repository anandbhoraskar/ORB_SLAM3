...
MapPoint::MapPoint():
    ...
    mpReplaced(static_cast<MapPoint*>(NULL)), isDynamic(false)
{
...
MapPoint::MapPoint(const cv::Mat &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mnOriginMapId(pMap->GetId()), isDynamic(false)
{
...
MapPoint::MapPoint(const cv::Mat &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnFound(1), mbBad(false), mpReplaced(NULL), mpMap(pMap), mnOriginMapId(pMap->GetId()), isDynamic(false)
{
    Pos.copyTo(mWorldPos);
    cv::Mat Ow;
